...
async function getAllUser(req, res) {
  try {
    const users = await db
      ...
      .select("*")
      .where(req.body)
      .andWhereNot("email", "=", `andromeda@gmail.com`);
...
async function updateUser(req, res) {
  try {
    ...
    return res
      .status(203)
      .send({ success: false, message: "May be the emaill is occupied." });
  }
}
...
async function user_search(req, res) {
  try {
    ...
    } else user = await db("users").select("*");
  ...
  try {
    ...
    if (user) {
      user.map((row) => {
        ...
        if (row.role.includes("BUH")) meta.BHU += 1;
        else if (row.role.includes("Senior_Manager")) meta.Senior_Manager += 1;
        else if (row.role.includes("Manager")) meta.Manager += 1;
        else if (row.role.includes("Finance")) meta.Finance += 1;
        else if (row.role.includes("Operations")) meta.Operations += 1;
      });
    }
