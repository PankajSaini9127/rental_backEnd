...
async function get_old_agreement_finance(req, res) {
  try {
    const data = await db("landlords")
      .select("landlords.*", "agreements.code", "landlords.id as landlord_id","oldAg.*")
      .join("agreements", "landlords.agreement_id" , "=","agreements.id")
      .join("agreements as oldAg", "oldAg.code", "=","agreements.code")
      .whereNull("oldAg.type")
      .where("landlords.id","=", req.query.id)
